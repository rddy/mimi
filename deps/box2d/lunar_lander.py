...
class LunarLander(gym.Env, EzPickle):
    ...
    def reset(self):
        ...
        self.helipad_idx = np.random.choice(list(range(1, CHUNKS-1)))
        self.helipad_x1 = chunk_x[self.helipad_idx-1]
        self.helipad_x2 = chunk_x[self.helipad_idx+1]

        self.helipad_y = H/4
        height[self.helipad_idx-2] = self.helipad_y
        height[self.helipad_idx-1] = self.helipad_y
        height[self.helipad_idx+0] = self.helipad_y
        height[self.helipad_idx+1] = self.helipad_y
        height[self.helipad_idx+2] = self.helipad_y
    ...
    def step(self, action):
        ...
        if not self.lander.awake:
            ...
            reward = +100
            succ = True
        return np.array(state, dtype=np.float32), reward, done, {'succ': succ}
